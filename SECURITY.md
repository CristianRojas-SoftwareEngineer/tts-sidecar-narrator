...
- Los eventos `Notification`, `SubagentStop`, `StopFailure` y `UserPromptSubmit` **nunca** usan el modo `llm`: da igual la configuración de `messageMode` o las claves presentes, su locución es siempre un aviso pre-sintetizado del catálogo (`src/message/static-avisos.ts`), reproducido con `speech play` y sin invocar ningún proveedor. Es una barrera estructural, no una omisión: `Notification` en particular suele dispararse en contextos sensibles (peticiones de permiso, alertas) y su texto ya viene redactado por Claude Code, así que no hay valor en parafrasearlo y sí riesgo en enviarlo a un tercero. En consecuencia, en modo `llm` el único evento que puede llegar a salir a la red es `Stop`, y solo si hay material narrable y una clave configurada.
